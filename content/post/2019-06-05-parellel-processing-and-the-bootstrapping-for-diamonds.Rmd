...
coefs %>% 
  select(-iteration) %>%
  summarise_all(funs(list(quantile(., probs = c(0.25, 0.95))))) %>%
  unnest %>%
  transpose %>%
  setNames(., c('25%','95%')) %>%
  map_df(unlist) %>%
  bind_cols(data.frame(vars = names(coefs %>% select(-iteration))), .)
## Thank you Stackoverflow for this code
## https://stackoverflow.com/questions/43373568/dplyr-to-iterate-over-all-columns-for-quantile
```
