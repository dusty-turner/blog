...
I am not sharing my code to improve readability.  If you are interested in the code for this script, send me a note, as I am happy to share.  
...
The data I use for this post comes from two leages - one from yahoo and one from ESPN.  Here it is below.
...
Below are the weekly results for your peruseal.
...
And on that note, who scored the most and lest points?
...
With that, how did this play out with overall wins and losses?
...
We already have win percentage.  Lets calculate the 'every team every week' win percentage.
...
oneteameachrow %>%
  ...
  geom_abline(slope = 1) +
  geom_text_repel(aes(label=paste(Team,wins)),hjust=.5, vjust=-.5, max.iter = 1000000) +
  annotate("text", x = .2, y = .8, label = "Luckly", size = 5) +
  annotate("text", x = .7, y = .2, label = "Unuckly", size = 5) +
