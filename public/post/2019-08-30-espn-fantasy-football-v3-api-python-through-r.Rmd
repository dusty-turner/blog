...
So, my motto here, to butcher a quote from "Tropic Thunder" is to, "never go full Python...."  In light of that, it gave me a great  opportuinty to learn `retriculate`.

### Access Private League Data

Take the steps below to acces private leagues.  Again, if your league is public, you can access the API like [so](https://dusty-turner.netlify.com/post/espn-fantasy-football-v3-api-an-r-solution/).

#### Build URL

To begin, we build the URL to access the API.  This is the same as if we access a public league.  Restating from my previous post - replace the `leagueID` with your private leage ID.  In the 'tail' of the URL, we request different chunks of data from the API. These are all the elements of which I’m aware.  Feel free to shorten if you do not need to request all the data.
...
swid = "{cookie-with-curley-brackets-included}"
...
Next, install the `reticulate` package with `install.packages("reticulate")` and call the library.  
```{r}
library(reticulate)
```
...
**Note** The code below is Python.  According to the `reticulate` documentation, R and Python will pass variables back and forth betwen each other.  Pass R objects to Python by preceeding the r variable with `r.`. Python objects pass to R by preceeding the object with `py$`.
...
r = requests.get(r.url,
                 cookies={"SWID": "{cookie-with-curley-brackets-included}",
...
The first basic piece of information to extact is team records.
