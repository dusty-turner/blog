---
title: "2005 Ford F150 Exploratory Data Analysis" 
author: Dusty Turner
date: '2018-06-29'
...
Many of the munging is made possible through the `lubridate` package.  To support this analysis, I wrote a post about it [here](https://dusty-turner.netlify.com/post/dates-in-r/)
...
Its a good practice to ensure all the names of your data are in the same format.  Lets use the `janitor` package and the `clean_names` function.  This ensures that the names are unique, consistent, have no non alpha-numeric characters, and transform spaces into underscores.  
...
This is more informative.  The `skim` function shows us the total number of observations and variables, missingness, descriptive statistics, and a general distribution of the data.
...
Also, for every mile I've driven total on my car, I should expect to receive ~.00001 miles per gallon less.  My layman's interpretation of this is that as my truck gets older, it gets worse gas mileage on average.  
...
First, I'll create a tibble of months from when we purchased the pick up until now. I'll extract out the month and year just like my original data-set.  
...
Next, I'll do a `full_join` of the helper tibble I created with my original data.  
...
One of the assumptions that I must make is that my time series is stationary.  The Dicky-Fuller (ADF) test (from the `tsries` package), tests the null hypothesis that the data is non-stationary.  The alternative hypothesis is that the data is stationary.
...
You can see below that there is a season aspect and trend component aspect that fluctuates over time.
